...
module.exports = function (req, res) {
  ...
  if (!cmd) {
    return res.send('sorry, ' + req.param('cmd') + ' is an invalid ender command. :(');
  }

  ender.exec(cmd, name, function (err) {
    if (err) {
      return res.send('sorry, ' + req.param('cmd') + ' is an invalid ender command. :( Check your spelling and try again!');
    }
